...
class ControllerPaymentmixplat extends Controller {
    ...
    public function index($payname = array('name' => 'mixplat')) {
        ...
        if ($this->config->get($pname . '_instruction_attach')) {
            ...
            $data['mixplati']         = $this->model_payment_mixplat->getCustomFields($order_info, $this->config->get($pname . '_instruction_' . $this->config->get('config_language_id')));
        }

        $data['pname'] = $pname;

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mixplat.tpl')) {
            return $this->load->view($this->config->get('config_template') . '/template/payment/mixplat.tpl', $data);
        } else {
            return $this->load->view('default/template/payment/mixplat.tpl', $data);
        }
    }
    ...
    public function callback() {
        ...
        if ($this->config->get($order_info['payment_code'] . '_createorder_or_notcreate') && $order_info['order_status_id'] != $this->config->get($order_info['payment_code'] . '_on_status_id')) {
            ...
            if (!$this->config->get($order_info['payment_code'] . '_mail_instruction_attach')) {
                ...
                $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get($order_info['payment_code'] . '_order_status_id'), $comment, true);
            } else {
                $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get($order_info['payment_code'] . '_order_status_id'), '', true);
            }
            ...
            if (!$this->config->get($order_info['payment_code'] . '_success_alert_customer')) {
                if ($this->config->get($order_info['payment_code'] . '_success_comment_attach')) {
                    ...
                    $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get($order_info['payment_code'] . '_order_status_id'), $message, true);
                } else {
                    $message = '';
                    $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get($order_info['payment_code'] . '_order_status_id'), $message, true);
                }
            ...
            if (!$this->config->get($order_info['payment_code'] . '_success_alert_customer')) {
                ...
                $this->model_checkout_order->addOrderHistory($order_info['order_id'], $this->config->get($order_info['payment_code'] . '_order_status_id'), '', false);
    ...
    public function go() {
        ...
        $data['merchant_url'] = $this->model_payment_mixplat->getCustomFields($order_info, 'gopay');
        ...
        if ($paystat['status'] == 0) {
            ...
            if ($this->config->get($order_info['payment_code'] . '_hrefpage_text_attach')) {
                $data['send_text'] = $this->model_payment_mixplat->getCustomFields($order_info, $this->config->get($order_info['payment_code'] . '_hrefpage_text_' . $this->config->get('config_language_id')));
            } else {
                $data['send_text'] = sprintf($this->language->get('send_text'), $order_info['order_id'], $this->model_payment_mixplat->getCustomFields($order_info, 'koplate'));
            }
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mixplat_go.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/mixplat_go.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/payment/mixplat_go.tpl', $data));
        }
    ...
    public function success() {
        ...
        if ($order_info['order_status_id'] == $this->config->get($order_info['payment_code'] . '_order_status_id')) {
            ...
            if ($this->config->get($order_info['payment_code'] . '_success_page_text_attach')) {
                ...
                $data['success_text'] .= htmlspecialchars_decode($this->model_account_yandexur->getCustomFields($order_info, $this->config->get($order_info['payment_code'] . '_success_page_text_' . $this->config->get('config_language_id')), $order_info['payment_code']));
            } else {
            ...
            if ($this->config->get($order_info['payment_code'] . '_waiting_page_text_attach')) {
                ...
                $data['success_text'] .= htmlspecialchars_decode($this->model_account_yandexur->getCustomFields($order_info, $this->config->get($order_info['payment_code'] . '_waiting_page_text_' . $this->config->get('config_language_id')), $order_info['payment_code']));
            } else {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mixplat_success.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/mixplat_success.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/payment/mixplat_success.tpl', $data));
        }
    }
    ...
    public function fail() {
        ...
        if ($this->config->get($order_info['payment_code'] . '_fail_page_text_attach')) {
            ...
            $data['fail_text'] .= htmlspecialchars_decode($this->model_payment_mixplat->getCustomFields($order_info, $this->config->get($order_info['payment_code'] . '_fail_page_text_' . $this->config->get('config_language_id')), $order_info['payment_code']));
        } else {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/mixplat_fail.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/payment/mixplat_fail.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/payment/mixplat_fail.tpl', $data));
        }
