...
class ModelExtensionPaymentMixplat extends Model {
    ...
    public function secondmodel($address, $total, $pname) {
        ...
        if ($total > 0) {
            ...
            if ($this->config->get('payment_'.$pname . '_status')) {
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int) $this->config->get('payment_'.$pname . '_geo_zone_id') . "' AND country_id = '" . (int) $address['country_id'] . "' AND (zone_id = '" . (int) $address['zone_id'] . "' OR zone_id = '0')");

                if (!$this->config->get('payment_'.$pname . '_geo_zone_id')) {
                    $status = true;
        ...
        if ($status) {
            ...
            $method_data = array(
                ...
                'title'      => $metname,
                'terms'      => '',
                'sort_order' => $this->config->get('payment_'.$pname . '_sort_order'),
            );
        }
    ...
    public function getProc($out_summ, $order_info) {
        ...
        if ($this->config->get('payment_'.$order_info['payment_code'] . '_komis')) {

            $totalrub['sum']   = number_format($out_summ * ($this->config->get('payment_'.$order_info['payment_code'] . '_komis') / 100) + $out_summ, 2, '.', '');
    ...
    public function getCustomFields($order_info, $varabliesd) {
        ...
        if ($this->config->get('payment_'.$pname . '_fixen')) {
            if ($this->config->get('payment_'.$pname . '_fixen') == 'fix') {
                $out_summ = $this->config->get('payment_'.$pname . '_fixen_amount');
            } else {
                $out_summ = $order_info['total'] * $this->config->get('payment_'.$pname . '_fixen_amount') / 100;
            }
        } else {
        ...
        foreach ($instros as $instro) {
            if ($instro == 'pay_link' || $instro == 'gopay' || $instro == 'koplate' || $instro == 'order_id' || $instro == 'amount' || $instro == 'fee' || $instro == 'amount_fee' || $instro == 'sum_fee') {
                ...
                if ($instro == 'fee') {
                    if ($this->config->get('payment_'.$pname . '_komis')) {
                        $instro_other = $this->config->get('payment_'.$pname . '_komis') . '%';
                    } else { $instro_other = '';}
                }

                if ($instro == 'amount_fee') {
                    if ($this->config->get('payment_'.$pname . '_komis')) {
                        $instro_other = $this->currency->format($out_summ * $this->config->get('payment_'.$pname . '_komis') / 100, $currency['currency_code'], $currency['currency_value'], true);
                    } else { $instro_other = '';}
                }

                if ($instro == 'sum_fee') {
                    if ($this->config->get('payment_'.$pname . '_komis')) {
                        $instro_other = $this->currency->format($out_summ + ($out_summ * $this->config->get('payment_'.$pname . '_komis') / 100), $currency['currency_code'], $currency['currency_value'], true);
                    } else { $instro_other = '';}
                }
